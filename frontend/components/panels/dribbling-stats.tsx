  ...
  return (
    <div className="animate-fade-in flex flex-col gap-8 pb-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          <h1 className="text-5xl font-black text-white tracking-tighter">Dribbling Intelligence</h1>
        ...
        <div className="flex gap-2">
          <div className="bg-white/5 border border-white/10 rounded-xl px-4 py-2">
            <p className="text-sm font-black text-white/30 tracking-widest">Global Success</p>
            <p className="text-2xl font-black text-[#006747]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          <h2 className="text-base font-black text-white/40 tracking-[0.2em] ml-1">Squad Overview</h2>
          {[0, 1].map((teamId) => {
            ...
            return (
              <Card key={teamId} className="glass-card p-6 border-white/5 relative overflow-hidden group">
                ...
                <div className="flex items-center justify-between mb-6 relative z-10">
                  <h3 className="text-2xl font-black text-white tracking-tighter">Team {String.fromCharCode(65 + teamId)}</h3>
                  <Badge className={`${getSuccessBgColor(stats.success_rate)} ${getSuccessColor(stats.success_rate)} border-none px-4 py-1.5 rounded-xl font-black text-base shadow-xl`}>
                    {stats.success_rate.toFixed(1)}%
                  </Badge>
                </div>
                <div className="grid grid-cols-3 gap-4 text-sm relative z-10">
                  <div className="bg-white/5 rounded-2xl p-4 border border-white/5">
                    <p className="text-white/30 text-sm font-black tracking-widest mb-1">Attempts</p>
                    <p className="text-white text-3xl font-black">{stats.total_dribbles}</p>
                  </div>
                  <div className="bg-white/5 rounded-2xl p-4 border border-white/5">
                    <p className="text-white/30 text-sm font-black tracking-widest mb-1">Success</p>
                    <p className="text-[#006747] text-3xl font-black">{stats.successful_dribbles}</p>
                  </div>
                  <div className="bg-white/5 rounded-2xl p-4 border border-white/5">
                    <p className="text-white/30 text-sm font-black tracking-widest mb-1">Distance</p>
                    <p className="text-white text-3xl font-black">{(stats.total_distance_m / 1000).toFixed(1)}<span className="text-base font-bold text-white/30 ml-0.5">km</span></p>
                  </div>
                </div>
              </Card>
            )
        ...
        <Card className="glass-card p-6 border-white/5 flex flex-col">
          <h3 className="text-base font-black text-white tracking-widest mb-6">Success Distribution (Team A)</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="glass-card p-6 border-white/5">
          <h3 className="text-xs font-black text-white tracking-widest mb-6">Team Comparison</h3>
        ...
        <Card className="glass-card p-6 border-white/5">
          <h3 className="text-base font-black text-white tracking-widest mb-6">Top Dribblers %</h3>
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={topDribblers} layout="vertical">
                ...
                <YAxis dataKey="name" type="category" stroke="white" opacity={0.6} fontSize={12} width={80} axisLine={false} tickLine={false} />
                <Tooltip
                  contentStyle={{ backgroundColor: "#000000", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "12px" }}
                />
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-white/[0.02]">
                ...
                <th className="px-6 py-4 text-center text-sm font-black text-white/30 tracking-widest">Success</th>
                <th className="px-6 py-4 text-center text-sm font-black text-white/30 tracking-widest">Distance</th>
                <th className="px-6 py-4 text-center text-sm font-black text-white/30 tracking-widest">Bypassed</th>
              </tr>
            </thead>
              ...
              {playerStats.map((player) => (
                <tr key={player.player_id} className="hover:bg-white/[0.03] transition-colors group">
                  ...
                  <td className="px-6 py-4 text-center">
                    <Badge className={`${getSuccessBgColor(player.success_rate)} ${getSuccessColor(player.success_rate)} border-none px-3 py-1 rounded-lg font-black text-sm`}>
